...
The indentation convention is enforced by code convention only — there is no structural guarantee. The correct long-term solution is a central `PSScriptBuilderVerboseRenderer` class with static `Indent()`, `Outdent()`, and `Write()` methods. Workers would pass flat messages; the renderer would apply the correct indentation based on current depth.

This migration is tracked in `internal/TODO.md` (see **Verbose Renderer**).

Blockers for that migration:
- ~215 `Write-Verbose` calls across 47 files must be migrated
- ~35–50 `Indent/Outdent` pairs with `try/finally` blocks must be introduced
- Static depth state can drift after caught exceptions in interactive sessions
