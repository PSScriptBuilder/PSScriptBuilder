...
flowchart TD
    ...
    H --> I[GitHub Actions: ci.yml]
    I --> J[PSScriptAnalyzer + Pester Tests on 4 platforms]
    J --> K{Tests pass?}
...
sequenceDiagram
    ...
    alt Merge to main
        ...
        GH->>GH: Tag event triggers cd.yml
    end
```
...
Every merge to `main` on GitLab triggers the GitLab CI pipeline. Once `pester-tests` and `secret_detection` pass, the `mirror-to-github` job pushes the current state to GitHub. That push event on GitHub `main` triggers `ci.yml`.

`ci.yml` runs two jobs sequentially: first `analyze.yml` (PSScriptAnalyzer), then `test.yml` (Pester on 4 platforms). This validates every merged change against the full platform matrix (PS5.1, PS7 on Windows, Linux, macOS) — coverage that the GitLab pipeline alone (PS7/Linux only) does not provide.

```mermaid
flowchart LR
    P[push to main] --> CI[ci.yml]
    CI --> A[analyze.yml<br/>PSScriptAnalyzer]
    A --> T[test.yml<br/>4 platforms]
```

**Release (`git push --follow-tags`):**

`git push --follow-tags` sends both the branch and the annotated tag to GitLab in a single transaction. GitLab starts two pipelines in parallel — one for the branch push, one for the tag push. Both run `pester-tests` and `mirror-to-github`. The tag pipeline additionally pushes the tag to GitHub, which triggers `cd.yml`.

`cd.yml` is the release orchestrator. It runs four jobs sequentially:

1. **`analyze`** — calls `analyze.yml` via `workflow_call`. Runs PSScriptAnalyzer. All subsequent jobs are blocked if this fails.
2. **`test`** — calls `test.yml` via `workflow_call`. Runs the full Pester test suite on all 4 platforms. Blocked if `analyze` failed.
3. **`publish-to-psgallery`** — runs `publish.ps1` to upload `build/Output/` to the PowerShell Gallery using `PSGALLERY_API_KEY`. Blocked if `test` failed.
4. **`create-release`** — extracts the release notes for the current version from `CHANGELOG.md` and creates a GitHub Release with that content. Blocked if `publish-to-psgallery` failed — a GitHub Release is only created when the module has actually been published.

Note: `test.yml` runs twice during a release — once via `ci.yml` (triggered by the branch push) and once via `cd.yml` (triggered by the tag). This is an accepted trade-off of the `--follow-tags` push strategy and does not affect correctness.

```mermaid
flowchart TD
    FT[git push --follow-tags]
    FT --> CD[cd.yml]
    CD --> A[analyze.yml<br/>PSScriptAnalyzer]
    A -->|needs analyze| T[test.yml<br/>4 platforms]
    T -->|needs test| PUB[publish-to-psgallery]
    PUB -->|needs publish| CR[create-release]
...
Jobs (in order):
1. `analyze` — calls `analyze.yml` via `workflow_call`
2. `test` — calls `test.yml` via `workflow_call` (needs: analyze)
...
1. `analyze` — calls `analyze.yml` via `workflow_call`
2. `test` — calls `test.yml` via `workflow_call` (needs: analyze)
3. `publish-to-psgallery` — runs `publish.ps1` to publish to PowerShell Gallery using `PSGALLERY_API_KEY` (needs: test)
4. `create-release` — extracts release notes from `CHANGELOG.md` and creates a GitHub Release (needs: publish-to-psgallery)

### analyze.yml

Triggered by: `workflow_call` (from ci.yml and cd.yml)

Runs PSScriptAnalyzer on `ubuntu-latest`.
...
| `PSGALLERY_API_KEY` | GitLab CI Variable + GitHub Secret | `cd.yml` |
