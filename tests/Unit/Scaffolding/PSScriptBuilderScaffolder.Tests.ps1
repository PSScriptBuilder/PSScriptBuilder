...
Describe 'PSScriptBuilderScaffolder' {
    ...
    Context 'Scaffold - build script content' {
        ...
        It 'Should contain the build-executed guard variable' {
            $script:buildScriptContent | Should -Match 'ContentProjectBuildExecuted'
        }

        It 'Should contain a Write-Warning in the guard block' {
            $script:buildScriptContent | Should -Match 'Write-Warning'
        }

        It 'Should set the guard variable to $true' {
            $script:buildScriptContent | Should -Match '\$Global:ContentProjectBuildExecuted\s*=\s*\$true'
        }
    }
