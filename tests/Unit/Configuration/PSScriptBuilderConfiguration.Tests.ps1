...
'@
    ...
    Context 'CreateDefault' {
        ...
        It 'Should return the absolute path to the created configuration file' {
            $root       = Join-Path $TestDrive 'cd-return'
            New-Item -Path $root -ItemType Directory -Force | Out-Null
            $expected   = Join-Path $root 'psscriptbuilder.config.json'

            $result = [PSScriptBuilderConfiguration]::CreateDefault($root, $false)
