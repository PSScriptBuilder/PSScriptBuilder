...
Describe 'PSScriptBuilderContentCollector' {
    ...
    Context 'GetProcessedFiles' {
        ...
        It 'Should return empty array when no collectors are registered' {
            $cc = New-ContentCollector
            $result = $cc.GetProcessedFiles()
            $result.Count | Should -Be 0
        }

        It 'Should return the source file for a collected class' {
            $file = New-TestFile 'GPF-Class.ps1' 'class GpfClass { }'
            $cc = New-ContentCollector
            $cc.AddCollector((New-ClassCollectorWithFile -FilePath $file))
            $cc.Execute()
            $result = $cc.GetProcessedFiles()
            $result | Should -Contain $file
        }

        It 'Should return the source file for a collected enum' {
            $file = New-TestFile 'GPF-Enum.ps1' 'enum GpfEnum { Val }'
            $cc = New-ContentCollector
            $cc.AddCollector((New-EnumCollectorWithFile -FilePath $file))
            $cc.Execute()
            $result = $cc.GetProcessedFiles()
            $result | Should -Contain $file
        }

        It 'Should return the source file for a collected function' {
            $file = New-TestFile 'GPF-Func.ps1' 'Function Get-GpfFunc { }'
            $cc = New-ContentCollector
            $cc.AddCollector((New-FunctionCollectorWithFile -FilePath $file))
            $cc.Execute()
            $result = $cc.GetProcessedFiles()
            $result | Should -Contain $file
        }
        ...
        It 'Should deduplicate files appearing in multiple collectors' {
            $sharedFile = New-TestFile 'GPF-Shared.ps1' "class SharedClass { }`nenum SharedEnum { A }"
            $cc = New-ContentCollector
            $cc.AddCollector((New-ClassCollectorWithFile -Key 'Classes' -FilePath $sharedFile))
            $cc.AddCollector((New-EnumCollectorWithFile  -Key 'Enums'   -FilePath $sharedFile))
            $cc.Execute()
            $result = $cc.GetProcessedFiles()
            ($result | Where-Object { $_ -eq $sharedFile }).Count | Should -Be 1
        }
    }
}
