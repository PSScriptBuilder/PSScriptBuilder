...
Describe 'PSScriptBuilderDependencyAnalyzer - Analyze' {
    ...
    Context 'Analyze - component name conflict' {

        It 'Should throw InvalidOperationException when a Class and a Function share the same name' {
            $analyzer = New-Analyzer @(
                (New-ClassCollector @("$ClassesPath\ConflictNameClass.ps1")),
                (New-FuncCollector  @("$FuncsPath\ConflictNameFunction.ps1"))
            )

            { $analyzer.Analyze() } | Should -Throw '*ConflictName*'
        }
    }
}
