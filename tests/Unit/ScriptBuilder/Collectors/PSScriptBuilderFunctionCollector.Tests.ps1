...
Describe 'PSScriptBuilderFunctionCollector' {
    ...
    Context 'Collection - basic' {
        ...
        It 'Should collect a single function from a file' {
            $file = New-TestFile 'SingleFunc.ps1' @'
...
'@
            ...
            $collector.FunctionData.ContainsKey('Get-Something') | Should -BeTrue
        }

        It 'Should collect multiple functions from a single file' {
            $file = New-TestFile 'MultiFuncs.ps1' @'
Function Get-Alpha {
    [CmdletBinding()]
    param()
}
...
'@
        ...
        It 'Should not collect class methods as standalone functions' {
            $file = New-TestFile 'ClassWithMethod.ps1' @'
class MyClass {
    [string] GetName() {
        return "test"
    }
...
'@
    ...
    Context 'TryGetComponentDetail' {
        ...
        It 'Should include project-internal called functions in Dependencies' {
            $fileA = New-TestFile 'FuncDet-CallerHelper.ps1' "Function Get-Helper { [CmdletBinding()] param() }"
            $fileB = New-TestFile 'FuncDet-CallerMain.ps1' @'
Function Get-WithCalls {
    [CmdletBinding()]
    param()
    Get-Helper
}
'@
            ...
            $knownComponents.Add('Get-Helper') | Out-Null
            $knownComponents.Add('Get-WithCalls') | Out-Null

            $result = $collector.TryGetComponentDetail('Get-WithCalls', $knownComponents)

            $result.Dependencies | Should -Contain 'Get-Helper'
        }

        It 'Should not include external called functions in Dependencies' {
            $file = New-TestFile 'FuncDet-ExtCalls.ps1' @'
Function Get-WithExternalCalls {
    [CmdletBinding()]
    param()
    Get-Item -Path "C:\temp"
}
'@
        ...
        It 'Should not include a self-reference in Dependencies' {
            $file = New-TestFile 'FuncDet-Self.ps1' @'
