...
```
Remove-PSScriptBuilderCollector [-ContentCollector] <PSScriptBuilderContentCollector> [-CollectionKey] <String>
 [<CommonParameters>]
```

## DESCRIPTION
The Remove-PSScriptBuilderCollector cmdlet removes a collector from a ContentCollector instance
based on its CollectionKey.
The cmdlet supports pipeline input and returns the modified
...
This is a RAM-only operation that is fully reversible.
Collectors can be re-added at any time
...
### -ContentCollector
The ContentCollector instance to remove the collector from.
...
### -CollectionKey
The unique identifier (CollectionKey) of the collector to remove.
Must match exactly
(case-insensitive).
...
calls PSScriptBuilderCollectorCollection.Remove().
The collection uses case-insensitive
...
(Add-PSScriptBuilderCollector does not have WhatIf support either).
