...
module's current version and release metadata — build timestamp, Git commit hash, branch
...
```json title="psscriptbuilder.bumpconfig.json"
{
    "bumpFiles": [
        {
            "description": "Module template — inject version and build metadata",
            "path": "build\\Templates\\HRModule.psm1.template",
...
```powershell title="HRModule.psm1.template"
    ...
    "bumpFiles": [
        {
            ...
            "path": "build\\Output\\HRModule.psd1",
            "items": [
                {
...
```powershell
.\Build-HRModule.ps1 -Release -Patch
```
