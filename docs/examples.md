...
on every `.ps1` change. The optional `-Action` parameter receives the
`PSScriptBuilderBuildResult` after each successful build — useful for running tests or printing
a summary.
