...
```powershell title="Inspect collected class definitions"
...
| `File`         | `PSScriptBuilderFileData`      | `Name`, `SourceCode` |
...
```powershell title="List all derived classes"
...
```powershell title="Find subclasses of a specific base class"
...
```powershell title="Find classes that reference a specific type"
$classes |
...
```powershell title="Assert expected classes are present"
...
```powershell title="Find classes with static initializer references"
$classes |
...
```powershell title="Find functions with outgoing function calls"
...
```powershell title="Find functions not called by any other function"
...
```powershell title="Rank classes by number of dependencies"
$classes |
...
```powershell title="Get source code of a single class"
...
```powershell title="Run dependency analysis"
...
affected by a change). See the [Dependency Analysis Guide](dependency-analysis.md) for a
complete walkthrough of all result properties and use cases.
...
```powershell title="Analyse template placeholders"
$analysis = New-PSScriptBuilderContentCollector |
    Add-PSScriptBuilderCollector -Type Class -IncludePath "src\Classes" |
    Get-PSScriptBuilderTemplateAnalysis -TemplatePath ".\build\Templates\MyModule.psm1.template"
