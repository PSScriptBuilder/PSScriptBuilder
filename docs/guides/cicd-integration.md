...
3. **Run the build script** — invoke your `Build-MyScript.ps1` (or equivalent); the script
...
!!! tip "Keep the build script outside CI configuration"
    Define your full build logic in a dedicated PowerShell script (e.g. `Build-MyScript.ps1`)
...
and delegates the actual build logic to `Build-MyScript.ps1`. Keeping the build logic in a
...
To persist the build summary as a CI artifact — useful for debugging, auditing, or tracking
changes between builds — capture the result and pass it to
[`Export-PSScriptBuilderBuildResult`](../cmdlets/Export-PSScriptBuilderBuildResult.md):
...
Use `-Detailed` to include per-component details — type, name, source file, and dependencies
— in the JSON output. Without it, only the build summary and component counts are written.
...
New-PSScriptBuilderContentCollector |
    ...
    Compress-PSScriptBuilderScript `
        -RemoveComments `
        -RemoveBlankLines `
        -DestinationPath 'deploy\MyScript.ps1' `
        -Force
```
...
- [Build Pipeline](build-pipeline.md) — full build pipeline documentation
