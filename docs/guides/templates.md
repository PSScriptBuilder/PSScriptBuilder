...
Instead of writing the template by hand, use
...
New-PSScriptBuilderTemplate -ContentCollector $contentCollector `
                            -OutputPath       "build\Templates\MyScript.ps1.template"
```

The generated file uses the correct placeholder structure for your codebase: one placeholder
per registered collector in Free Mode, or a single `{{ORDERED_COMPONENTS}}` block when
cross-dependencies are detected. The result is ready to use immediately.
...
if (Test-PSScriptBuilderTemplate -ContentCollector $contentCollector -TemplatePath 'build\Templates\MyProject.ps1.template') {
...
```powershell
$analysis = Get-PSScriptBuilderTemplateAnalysis -ContentCollector $contentCollector -TemplatePath 'build\Templates\MyProject.ps1.template'
    ...
    "bumpFiles": [
        {
            ...
            "path": "build\\Templates\\MyScript.ps1.template",
