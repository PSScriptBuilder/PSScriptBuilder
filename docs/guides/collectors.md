...
Collectors scan files, not individual definitions. Every file matching the configured paths and filters
is read and all definitions of the relevant type it contains are extracted individually. A single file may contain multiple class
definitions, multiple function definitions, or multiple enum definitions — the collector
handles all of them transparently. There is no requirement to keep one definition per file.
A single file may even mix different definition types — a class and a function in the same file
are handled correctly. Each collector extracts only its own component type, leaving the rest
untouched. See [Example 04](../examples.md#04-flexible-file-structure) for a demonstration.
...
```powershell title="First collector"
...
Most builds need multiple collectors — classes, functions, and using statements at minimum:

```powershell title="Multiple collectors"
$contentCollector = New-PSScriptBuilderContentCollector |
...
By default, each collector maps to a `{{Token}}` in your template using its type's default key
(e.g. `{{CLASS_DEFINITIONS}}`). Use `-CollectionKey` to assign a custom key — this is required
when you need multiple collectors of the same type for different source directories:

```powershell title="Custom keys for multiple collectors of the same type"
$contentCollector = New-PSScriptBuilderContentCollector |
...
```powershell title="Matching template tokens"
{{CORE_CLASSES}}
{{DOMAIN_CLASSES}}
```
...
```powershell title="Exclude files"
$contentCollector = New-PSScriptBuilderContentCollector |
...
```powershell title="Exclude a subdirectory"
$contentCollector = New-PSScriptBuilderContentCollector |
...
```powershell title="Disable recursive scanning"
$contentCollector = New-PSScriptBuilderContentCollector |
...
```powershell title="Include specific files"
$contentCollector = New-PSScriptBuilderContentCollector |
...
After building or calling `Collect()` directly, inspect what was collected.

**List all registered collectors:**

```powershell title="List collectors"
Get-PSScriptBuilderCollector -ContentCollector $contentCollector
```

**Get a specific collector by key:**

```powershell title="Get a specific collector"
...
```powershell title="Inspect collected data"
...
```powershell title="Get a single item"
...
Use [`Remove-PSScriptBuilderCollector`](../cmdlets/Remove-PSScriptBuilderCollector.md) to remove a registered collector by key. The operation is
idempotent — removing a non-existent key produces a warning, not an error:

```powershell title="Remove and replace a collector"
...
- Non-existent `-IncludePath` values produce a warning at collector creation time, not an error — the collector is still created. At collection time, a missing path that was accepted with a warning will throw an error.
...
- [Templates Guide](templates.md) — how collectors map to template tokens
