...
class PSScriptBuilderScaffolder {
    ...
    hidden [string] CreateBuildScript([string] $projectPath) {
        ...
        $guardName    = "$($this.Request.Name -replace '[^a-zA-Z0-9]', '')BuildExecuted"

        $stringBuilder = [StringBuilder]::new()

        [void] $stringBuilder.AppendLine('using module PSScriptBuilder')
        [void] $stringBuilder.AppendLine('')
        [void] $stringBuilder.AppendLine("if (`$Global:$guardName) {")
        [void] $stringBuilder.AppendLine("    Write-Warning `"$scriptName has already been executed in this session. Start a new PowerShell session to run it again.`"")
        [void] $stringBuilder.AppendLine('    return')
        [void] $stringBuilder.AppendLine('}')
        [void] $stringBuilder.AppendLine('')
        [void] $stringBuilder.AppendLine("`$Global:$guardName = `$true")
        [void] $stringBuilder.AppendLine('')
