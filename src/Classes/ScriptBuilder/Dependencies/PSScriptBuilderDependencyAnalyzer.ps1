...
.DESCRIPTION
    ...
    2. Component Name Validation: Ensures no Class and Function share the same name
    3. Graph Building: Constructs a dependency graph from collected components
    4. Cycle Detection: Identifies circular dependencies
    5. Topological Sorting: Orders components by dependencies (if no cycles)
    6. Enum Stabilization: Ensures enums appear before classes and functions (if no cycles)
    7. Cross-Dependency Detection: Identifies if component types are intermixed (if no cycles)
...
class PSScriptBuilderDependencyAnalyzer {
    ...
    .DESCRIPTION
        ...
        2. Validates that no Class and Function share the same name
        3. Gathers component statistics
        4. Builds dependency graph from collected components
        5. Detects circular dependencies
        6. Sorts components topologically (if no cycles)
        7. Stabilizes order: ensures enums appear before classes and functions (if no cycles)
        8. Detects cross-dependencies between component types (if no cycles)
    ...
    [PSScriptBuilderDependencyAnalysisResult] Analyze() {
        try {
            ...
            # Step 2: Validate component name uniqueness (Class vs Function conflict)
            $this.ValidateComponentNameUniqueness()

            # Step 3: Gather component statistics (result of collection)
            ...
            # Step 6: Sort topologically
            $orderedComponents = $this.SortTopologically($graph)

            # Step 7: Stabilize — enums always before classes and functions
            $orderedComponents = $this.StabilizeEnumsFirst($orderedComponents)

            # Step 8: Detect cross-dependencies
    ...
    .DESCRIPTION
        Delegates to PSScriptBuilderContentCollector.Execute() to gather all components from source files.
    ...
    .DESCRIPTION
        Delegates to PSScriptBuilderContentCollector.ValidateComponentNameUniqueness().
        Called after content collection and before graph building to ensure the dependency
        graph has unambiguous node keys.
    #>
